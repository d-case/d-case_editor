...
import net.dependableos.dcase.diagram.providers.DcaseMarkerNavigationProvider;
import net.dependableos.dcase.diagram.providers.DcaseValidationProvider;
...
public class ValidateAction extends Action {
    ...
    // AUTO_GENERATED:END
    /**
     * A thread-local manages validators.
     */
    public static final ThreadLocal<DcaseValidator> VALIDATOR_COLLECTION = new ThreadLocal<DcaseValidator>();
    // AUTO_GENERATED:START
    ...
    public static void runValidation(View view) {
        try {
            if (DcaseDiagramEditorUtil.openDiagram(view.eResource())) {
    ...
    public static void runValidation(DiagramEditPart diagramEditPart, View view) {
        ...
        Argument argument = (Argument) diagram.getElement();
        final XMLResource xmlResource = (XMLResource) argument.eResource();

        DcaseValidationProvider.runWithConstraints(txDomain, new Runnable() {

            public void run() {

                // creates a validator and sets it to the thread-local.
                LinkManager linkManager = new LinkManager();
                linkManager.load(xmlResource);
                DcaseValidator dcaseValidator = new DcaseValidator(linkManager);
                VALIDATOR_COLLECTION.set(dcaseValidator);

                validate(fpart, fview);

                // removes the validator.
                VALIDATOR_COLLECTION.remove();
            }
        });
    ...
    private static void validate(DiagramEditPart diagramEditPart, View view) {
        ...
        if (target != null) {
            DcaseMarkerNavigationProvider.deleteMarkers(target);
        }
    ...
    private static void addMarker(EditPartViewer viewer, IFile target,
        ...
        DcaseMarkerNavigationProvider.addMarker(target, elementId, location,

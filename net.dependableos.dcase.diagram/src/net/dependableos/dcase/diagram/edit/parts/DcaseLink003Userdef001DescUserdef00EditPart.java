 ...
 * @generated
 */
public class DcaseLink003Userdef001DescUserdef00EditPart extends LabelEditPart
    ...
    static {
        registerSnapBackPosition(
                DcaseVisualIDRegistry
                        .getType(net.dependableos.dcase.diagram.edit.parts.DcaseLink003Userdef001DescUserdef00EditPart.VISUAL_ID),
                new Point(0, 40));
    }
     ...
     * @generated
     */
    public DcaseLink003Userdef001DescUserdef00EditPart(View view) {
        super(view);
    }
    ...
    /**
     * Returns the attachment name or icon name.
     * @param isText whether attachment or not(icon).
     * @return the attachment name or icon name.
     */
    private String getResponsibilityStr(boolean isText) {
    	String text = null;
        ...
        if (text != null && text.length() > 0) {
        	...
        	if (textArray.length > 1) {
        		return isText ? textArray[0]:textArray[1];
        	}
        	if (textArray.length > 0) {
        		text = textArray[0]; // argument=text, icon=null
        	} else {
        		text = null; // argument=null, icon=null when text == ";"
        	}
        }
        return isText ? text:null; // argument=text, icon=null
    }
    /**
     * @generated NOT
     */
    protected Image getLabelIcon() {
    	String iconStr = getResponsibilityStr(false);
    	if (iconStr != null && iconStr.length() > 0) {
    		if (iconStr.indexOf("R") >= 0) { //$NON-NLS-1$
    			URL imageUrl = (URL)DcaseEditPlugin.getPlugin().getImage("full/obj16/Responsibility"); //$NON-NLS-1$
    			return ImageDescriptor.createFromURL(imageUrl).createImage();
    		}
    		if (iconStr.indexOf("C") >= 0) { //$NON-NLS-1$
    			URL imageUrl = (URL)DcaseEditPlugin.getPlugin().getImage("full/obj16/Userdef006"); //$NON-NLS-1$
    			return ImageDescriptor.createFromURL(imageUrl).createImage();
    		}
    	}
    	return null;
    ...
    protected String getLabelText() {
        String text = getResponsibilityStr(true);
        if (text == null || text.length() == 0) {
            text = defaultText;
        }
        return text;
    ...
    protected DirectEditManager getManager() {
        if (manager == null) {
            setManager(new TextDirectEditManager(this, TextDirectEditManager
                    .getTextCellEditorClass(this), DcaseEditPartFactory
